...
従来のテンプレートファイルの代わりに、`ComponentFunc` 関数（`func(Node)`）をプログラム的に合成して HTML を組み立てます。型安全性と Go の全機能をテンプレート構築時に活用できます。
...
mux.Handle("GET /greet", g.HandlerFunc(func(r *http.Request) []g.ComponentFunc {
	...
	return []g.ComponentFunc{
		gd.Head(gd.Title("挨拶")),
		gd.Body(gd.H1(gp.Value("こんにちは、" + name + "！"))),
	}
}))
```
...
```
gerbera/
├── common.go          # ComponentFunc 型 (func(Node))、Tag()、Literal()、ExecuteTemplate()
├── element.go         # Node インターフェース、Element 構造体 (TagName, Key, ClassNames, Attr, Children, Value)
├── template.go        # Template: マウント/レンダリング制御、io.Reader
├── render.go          # 再帰的 HTML レンダリング
├── server.go          # Handler()、HandlerFunc() HTTP ハンドラ; NewServeMux()（非推奨）
...
└── example/           # サンプルアプリケーション
    ...
    └── sns/           # Twitter クローン（MySQL 使用、フルスタック例）
```

### コアコンセプト

**Node** — HTML 要素の抽象化。すべてのプロパティセッターと DOM ヘルパーは `Node` に対して操作します:
...
**Handler / HandlerFunc** — 静的・動的ページ用の HTTP ハンドラ:

```go
// 静的: コンポーネントは作成時に固定
mux.Handle("GET /about", g.Handler(aboutPage()...))

// 動的: リクエストごとにコンポーネントを構築
...
**LiveMount** — SSR ページ内に LiveView を埋め込みます:

```go
gl.LiveMount("/admin/notifications")               // 基本
gl.LiveMount("/chat", gl.WithMountID("chat-panel")) // カスタム ID 付き
```
...
| [sns](example/sns/) | Twitter クローン（MySQL） | :8930 | `cd example/sns && docker compose up` |
