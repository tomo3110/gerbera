...
- `MaterializeCSS` — `<head>` に Materialize CSS/JS を追加
...
`gc.MaterializeCSS()` は親の `<head>` 要素に Materialize CSS の CDN リンクを追加し、charset/viewport メタタグがなければ自動追加します。Bootstrap の代わりとして利用できます。
...
| 関数 | 説明 |
|------|------|
| `gc.MaterializeCSS()` | `<head>` に Materialize CSS/JS を追加 |
