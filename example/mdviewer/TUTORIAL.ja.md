...
- `gu.Theme()` — CSS カスタムプロパティによる UI コンポーネントライブラリのテーマシステム
- `gu.Button()` / `gu.Badge()` / `gu.Row()` — インラインスタイルを置き換えるビルド済み UI コンポーネント
...
ビューアーは UI コンポーネントライブラリの `gu.Theme()` でベーススタイリング（フォント、色、スペーシング）を提供し、`gs.CSS(mdCSS)` で Markdown 固有のスタイルを追加します:
...
gd.Head(
    ...
    gu.Theme(),       // CSS カスタムプロパティによるベーステーマ
    gs.CSS(mdCSS),    // Markdown 固有スタイルのみ
),
```

`gu.Theme()` は CSS カスタムプロパティ（`--g-bg`, `--g-text`, `--g-accent`, `--g-border` 等）を定義する `<style>` 要素を注入します。Markdown CSS はハードコードされた色の代わりにこれらの変数を参照します:

```css
.md-textarea {
  background: var(--g-bg-inset);    /* #fafbfc の代わり */
  color: var(--g-text);             /* #24292e の代わり */
  font-family: var(--g-font-mono);  /* ハードコードされたフォントスタックの代わり */
}
.md-preview a { color: var(--g-accent); }           /* #0366d6 の代わり */
.md-preview blockquote { color: var(--g-text-secondary); } /* #6a737d の代わり */
```

これにより、ビューアーは任意のテーマに自動適応します。`gu.ThemeWith(gu.DarkTheme())` に切り替えるだけで、CSS の変更なしにダークモードのビューアーが得られます。

### UI コンポーネントによるヘッダー

ヘッダーはインラインスタイルの代わりに `gu.Row()`、`gu.Button()`、`gu.Badge()` を使用します:
...
利点:
- `gu.Button("Save", gu.ButtonPrimary, gu.ButtonSmall)` がインラインスタイル 7 行を置換
- `gu.Badge("Saved", "dark")` が手動スタイルの `<span>` を置換
- `gu.Row(gu.AlignCenter, gu.GapSm)` がインラインフレックス CSS を置換
...
5. `gu.ThemeWith(gu.DarkTheme())` でダークテーマに切り替え、CSS 変数ベースのスタイルが自動的に適応することを確認する
